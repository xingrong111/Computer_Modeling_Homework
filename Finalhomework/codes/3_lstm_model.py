...
def build_lstm_model(seq_length, n_features):
    """构建LSTM模型（多变量输出：确诊+治愈+死亡）"""
    model = Sequential([
        LSTM(50, activation='relu', return_sequences=True, input_shape=(seq_length, n_features)),
        Dropout(0.2),
        LSTM(50, activation='relu'),
        Dropout(0.2),
        Dense(n_features)  # 输出3个：确诊、治愈、死亡
    ])
    model.compile(optimizer='adam', loss='mse', metrics=['mae'])
    return model


def forecast_future_multi(model, last_seq, scaler, seq_length, n_features, days=7):
    """
    多变量滚动预测未来多天
    参数:
        model: 训练好的LSTM模型
        last_seq: 最后seq_length天的数据 (seq_length, n_features)
        scaler: 归一化器
        seq_length: 序列长度
        n_features: 特征数
        days: 预测天数
    返回:
        预测结果 (days, n_features)
    """
    current_seq = last_seq.copy().reshape(1, seq_length, n_features)
    predictions = []
    ...
    for _ in range(days):
        ...
        # 更新序列：去掉最旧的一天，加入新的预测值
        new_row = np.concatenate([current_seq[0, 1:, :], pred_scaled.reshape(1, n_features)], axis=0)
        current_seq = new_row.reshape(1, seq_length, n_features)
...
def main():
    ...
    # 使用平滑后的每日新增数据（多变量：确诊+治愈+死亡）
    data = full_df[['new_confirmed_smooth', 'new_recovered_smooth', 'new_deaths_smooth']].values
    scaler = MinMaxScaler()
    data_scaled = scaler.fit_transform(data)
    ...
    print(f"训练样本: {X_train.shape[0]}, 测试样本: {X_test.shape[0]}")
    print(f"特征数（确诊+治愈+死亡）: {n_features}")
    ...
    for i, label in enumerate(labels):
        ...
        print(f"LSTM {label} - RMSE: {rmse:.2f}, MAE: {mae:.2f}, R²: {r2:.4f}")
    print("=" * 50)

    # 保存预测结果
    lstm_results = pd.DataFrame({
        'date': test_df['date'].iloc[SEQ_LENGTH:].reset_index(drop=True),
        'actual_confirmed': y_test_actual[:, 0],
        'predicted_confirmed': y_pred[:, 0],
        'actual_recovered': y_test_actual[:, 1],
        'predicted_recovered': y_pred[:, 1],
        'actual_deaths': y_test_actual[:, 2],
        'predicted_deaths': y_pred[:, 2]
    })
    lstm_results.to_csv('D:/Adaima/Computer_build_model/Finalhomework/result/data/lstm_results_multi.csv', index=False)
    print("多变量预测结果已保存")

    # 多步预测：7天和14天
    last_seq = data_scaled[-SEQ_LENGTH:]
    pred_7d = forecast_future_multi(model, last_seq, scaler, SEQ_LENGTH, n_features, days=7)
    pred_14d = forecast_future_multi(model, last_seq, scaler, SEQ_LENGTH, n_features, days=14)
    ...
    forecast_7d_df.to_csv('D:/Adaima/Computer_build_model/Finalhomework/result/data/lstm_forecast_multi_7d.csv',
                          index=False)

    forecast_14d_df = pd.DataFrame({
        'date': future_dates_14,
        'confirmed': pred_14d[:, 0],
        'recovered': pred_14d[:, 1],
        'deaths': pred_14d[:, 2]
    })
    forecast_14d_df.to_csv('D:/Adaima/Computer_build_model/Finalhomework/result/data/lstm_forecast_multi_14d.csv',
                           index=False)

    print("\n" + "=" * 50)
    print("未来7天预测（确诊/治愈/死亡）：")
    print(f"  日期: {future_dates_7.strftime('%Y-%m-%d').tolist()}")
    print(f"  确诊: {pred_7d[:, 0].round(0)}")
    print(f"  治愈: {pred_7d[:, 1].round(0)}")
    print(f"  死亡: {pred_7d[:, 2].round(0)}")
    print("=" * 50)
    print("未来14天预测（确诊/治愈/死亡）：")
    print(f"  日期: {future_dates_14.strftime('%Y-%m-%d').tolist()}")
    print(f"  确诊: {pred_14d[:, 0].round(0)}")
    print(f"  治愈: {pred_14d[:, 1].round(0)}")
    print(f"  死亡: {pred_14d[:, 2].round(0)}")
    print("=" * 50)

    # 保存训练历史（用于绘制Loss曲线）
    import pickle
    with open('D:/Adaima/Computer_build_model/Finalhomework/result/data/lstm_history_multi.pkl', 'wb') as f:
        pickle.dump(history.history, f)
    print("训练历史已保存")
if __name__ == "__main__":
    main()